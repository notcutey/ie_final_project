...
class Decoder(nn.Module):
    ...
    def forward(self, q, x):
        q_bn = self.bn1(q)
        cross_attn, attn_map = self.cross_attn(q_bn, x, x)
        q = q + self.drop_path(cross_attn)
        q = q + self.drop_path(self.mlp(q))
        q_bn = self.bn2(q)
        q = q + self.drop_path(self.self_attn(q_bn, q_bn, q_bn)[0])
        return q, attn_map
...
class Token_Refine(nn.Module):
    ...
    def forward(self, x: Tensor, attn_save: bool = False, attn_root: str = None, file_name: str = None):
        B, _, H, W = x.size()
...
class Token(nn.Module):
    def __init__(self, outputdim=1024, classifier_num=81313, mode='train'):
        super().__init__()
        self.outputdim = 1024
    ...
    def forward_test(self, x, attn_save: bool = False, attn_root: str = None, file_name: str = None):
        x = self.backbone(x)
        x = self.tr(x, attn_save, attn_root, file_name)
